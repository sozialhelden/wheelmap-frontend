...
test.describe('Logo Display and Contrast', () => {
  ...
  test('logo is visible in LIGHT mode', async ({ page }) => {
    ...
    const logoLink = page.getByRole('banner').getByRole('link', { name: 'Home' });
  ...
  test('logo text has POOR contrast in dark mode (BUG: Asana #1210099669460115)', async ({ page }) => {
  ...
  test('logo passes axe accessibility scan in light mode', async ({ page }) => {
    ...
    expect(results.violations, 'Light mode: Banner should have no accessibility violations').toEqual([]);
  });
  ...
  test('logo passes axe accessibility scan in dark mode', async ({ page }) => {
    ...
    expect(results.violations, 'Dark mode: Banner should have no accessibility violations').toEqual([]);
  });
});
