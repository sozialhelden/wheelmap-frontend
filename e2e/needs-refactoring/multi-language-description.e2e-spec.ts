import { type Page, test } from "@playwright/test"; // Helper to dismiss onboarding dialog
...
test.skip("principal railway station of Berlin works with multi-language description", async ({
  page,
}) => {
  await page.goto("/");
  ...
  await page.goto("/amenities/node:240109189");
  await page.waitForLoadState("networkidle");
  await page.getByPlaceholder("Search for place or address").click();
  await page
    .getByPlaceholder("Search for place or address")
    .fill("Berlin Central Station");
  await page.getByPlaceholder("Search for place or address").press("Enter");
  await page.waitForLoadState("networkidle");
  await page
    .getByRole("link", { name: "Berlin Central Station(Train" })
    .click();
...
test.skip("english description should be the first language displayed", async ({
  page,
}) => {
  await page.goto("/");
  await dismissOnboarding(page);
