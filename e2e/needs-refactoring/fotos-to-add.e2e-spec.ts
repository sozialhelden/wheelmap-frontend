...
test.skip("has base URL", async ({ page }) => {
  await page.goto("/");
});

test.skip("upload a photo", async ({ page }) => {
  try {
    await page.goto("/");
    //   await skipOnboarding(page);
    await page.goto("/amenities/way:30050297");
    ...
    await page
      ...
      .setInputFiles("./e2e/1000015934.jpg");
...
test.describe("when the menu is closed", () => {
  test.skip("has correct ARIA snapshot on desktops", async ({ page }) => {
    skipOnMobiles();
    await page.goto("/");
    //  await skipOnboarding(page);
  ...
  test.skip("has correct ARIA snapshot on mobiles", async ({ page }) => {
    skipOnDesktops();
    await page.goto("/");
    await skipOnboarding(page);
    await expect(page.getByRole("banner")).toMatchAriaSnapshot(`
