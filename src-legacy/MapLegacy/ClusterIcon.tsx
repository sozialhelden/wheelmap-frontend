...
import { interpolateWheelchairAccessibility } from "../../lib/util/colors";
import * as markers from "../icons/markers";
import Icon, { StyledIconContainer } from "../shared/Icon";
...
export default class ClusterIcon extends L.Icon {
  ...
  createIcon() {
    ...
    const {
      ...
      backgroundColor,
      accessibility,
    } = interpolateWheelchairAccessibility(propertiesArray);
    const actualBackgroundColor =
      this.options.backgroundColor || d3Color(backgroundColor);
    const actualForegroundColor =
      this.options.foregroundColor || d3Color(foregroundColor);
    const MarkerComponent =
      markers[`${accessibility}${withArrow ? "With" : "Without"}Arrow`];
        ...
        ReactDOM.render(
          <Icon {...commonOptions} category={iconNames[0]}>
            <MarkerComponent
              ...
              fill={actualBackgroundColor.toString()}
            />
          </Icon>,
          elem
        );
