...
export function findMostUsableLanguageTag(
  ...
  const sanitizedRequestedLanguageTag =
  ...
  const tagWithoutRegion = sanitizedRequestedLanguageTag?.slice(0, 2);
  ...
  const foundLanguageTag = compact(languageTagsToTry).find((tag) =>
