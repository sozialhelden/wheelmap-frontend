...
import type { OSMAPIErrorResponse } from "../osm/OSMFeature";
import type {
  HasTypeTag,
  TypeMapping,
  TypeTagged,
...
export type TypeTaggedEquipmentInfo =
  | TypeTaggedWithId<"ac:EquipmentInfo">
  | TypeTaggedWithId<"a11yjson:EquipmentInfo">;
export type TypeTaggedEntrance =
  | TypeTaggedWithId<"ac:Entrance">
  | TypeTaggedWithId<"a11yjson:Entrance">;
export type TypeTaggedPlaceInfo = (
  | TypeTaggedWithId<"ac:PlaceInfo">
  | TypeTaggedWithId<"a11yjson:PlaceInfo">
) &
  PlaceInfo & {
...
export type TypeTaggedPhotonSearchResultFeature =
  TypeTagged<"photon:SearchResult">;
...
const isTypeTaggedWithId =
  <S extends keyof TypeMapping>(...types: S[]) =>
  (obj: unknown): obj is TypeTaggedWithId<S> =>
    types.includes(obj?.["@type"]);

export const isPlaceInfo = isTypeTaggedWithId(
  "ac:PlaceInfo",
  "a11yjson:PlaceInfo",
);
export const isEquipmentInfo = isTypeTaggedWithId(
  "ac:EquipmentInfo",
  "a11yjson:EquipmentInfo",
);
export const isEntrance = isTypeTaggedWithId(
  "ac:Entrance",
  "a11yjson:Entrance",
);
...
export function getKey(feature: AnyFeature) {
  return isSearchResultFeature(feature)
