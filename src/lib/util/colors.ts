...
import type { YesNoLimitedUnknown } from "../model/ac/Feature";
...
export function coloredWhite(color: string, value = 0.5) {
...
export function brighter(color: string, value = 0.3) {
  return hsl(color).brighter(value).toString();
}

export function darker(color: string, value = 0.3) {
  return hsl(color).darker(value).toString();
}

export function alpha(color: string, value = 0.4) {
  const alphaColor = rgb(color);
  alphaColor.opacity *= value;
  return alphaColor.toString();
}

export function mixLab(color1: string, color2: string, ratio = 0.5) {
...
function calculateWheelchairAccessibility(
  ...
  const undefinedCount = accessibilityValues.filter(
    (c) => c === "unknown",
  ).length;
...
function getWheelchairAccessibilityColors(
  ...
  const averageAccessibilityForDefinedBackground =
    definedAccessibilityBackgroundColorScale(averageRatingForDefined);
...
export function interpolateWheelchairAccessibilityColors(
  ...
  const { definedCount, averageRatingForDefined, clampedDefinedRatio } =
