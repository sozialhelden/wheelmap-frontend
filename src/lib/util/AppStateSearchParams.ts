...
import type { YesNoLimitedUnknown, YesNoUnknown } from "../model/ac/Feature";
...
function parseStringParam<T extends string = string>(
  value: ParseValue,
): T | null | undefined {
  if (value === undefined) {
    return undefined;
...
function parseStringArrayParam<T extends string = string>(
  value: ParseValue,
): T[] | null | undefined {
  if (value === undefined) {
    return undefined;
...
export const parseSearchParams = (
  ...
  category: search.has("category")
    ? parseStringParam(search.get("category"))
    : undefined,
  q: search.has("q") ? parseStringParam(search.get("q")) : undefined,
  wheelchair: search.has("wheelchair")
    ? parseStringArrayParam(search.get("wheelchair"))
    : undefined,
  toilet: search.has("toilet")
    ? parseStringArrayParam(search.get("toilet"))
    : undefined,
});
