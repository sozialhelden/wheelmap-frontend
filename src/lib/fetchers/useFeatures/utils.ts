...
} from "../../typing/discriminators/osmDiscriminator";
...
export const makeFetchProperties = (
  featureId: FeatureId,
  { acBaseUrl, acAppToken, osmBaseUrl, osmAppToken }: Partial<FetchOptions>,
): FetchOneFeatureProperties => {
  if (isOSMIdWithTableAndContextName(featureId)) {
    ...
    const {
      properties: { table, element, value },
    } = getOSMRDFComponents(featureId);
    return {
