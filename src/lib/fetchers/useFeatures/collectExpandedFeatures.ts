...
export const collectExpandedFeaturesResult = (
  ...
  {
    requestedFeatures,
    additionalAcFeatures,
    additionalOsmFeatures,
  }: ReturnType<typeof useExpandedFeatures>,
) => {
  ...
  featureIds.forEach((featureId) => {
    ...
    if (requestedFeature?.feature?.["@type"] === "ac:PlaceInfo") {
      const collected = {
        origin: featureId,
        acFeature: (requestedFeature.feature ?? additionalAcFeature) as
          | TypeTaggedPlaceInfo
          | undefined,
        osmFeature: additionalOsmFeature?.feature as
          | TypeTaggedOSMFeature
          | undefined,
        requestedFeature: requestedFeature.feature as
          | TypeTaggedPlaceInfo
          | TypeTaggedOSMFeature,
    ...
    if (requestedFeature?.feature?.["@type"] === "osm:Feature") {
      const collected = {
        origin: featureId,
        acFeature: additionalAcFeature?.feature as
          | TypeTaggedPlaceInfo
          | undefined,
        osmFeature: (requestedFeature.feature ?? additionalOsmFeature) as
          | TypeTaggedOSMFeature
          | undefined,
        requestedFeature: requestedFeature.feature as
          | TypeTaggedOSMFeature
