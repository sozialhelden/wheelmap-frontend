...
interface FetchOneFeaturePropertiesInternal<
  Feature extends TypeTaggedOSMFeature | TypeTaggedPlaceInfo,
> {
...
interface FetchOneFeatureResultInternal<
  Feature extends TypeTaggedOSMFeature | TypeTaggedPlaceInfo,
> extends Omit<FetchOneFeaturePropertiesInternal<Feature>, "typeTag"> {
  feature: Feature;
}

export type FetchOneOsmFeatureProperties =
  FetchOneFeaturePropertiesInternal<TypeTaggedOSMFeature>;
export type FetchOnePlaceInfoProperties =
  FetchOneFeaturePropertiesInternal<TypeTaggedPlaceInfo>;
export type FetchOneFeatureProperties =
  | FetchOneOsmFeatureProperties
  | FetchOnePlaceInfoProperties;

export type FetchOneOsmFeatureResult =
  FetchOneFeatureResultInternal<TypeTaggedOSMFeature>;
export type FetchOnePlaceInfoResult =
  FetchOneFeatureResultInternal<TypeTaggedPlaceInfo>;
 ...
 */
export const genericFetcher = async <Result = unknown>(
  fetchUri: string,
): Promise<Result> => {
 ...
 */
export const composeFetchOneFeature = (
  keyProperties: Record<string, FetchOneFeatureProperties>,
) => {
...
export interface FetchOsmToAcFeatureResult
  extends FetchOsmToAcFeatureProperties {
 ...
 */
export const composeOsmToAcFetcher = (
  keyProperties: Record<string, FetchOsmToAcFeatureProperties>,
) => {
  const osmToAcFetcher = async (
    fetchUri: string,
  ): Promise<FetchOsmToAcFeatureResult> => {
    const result =
      await genericFetcher<
        AccessibilityCloudAPIFeatureCollectionResult<PlaceInfo>
      >(fetchUri);
