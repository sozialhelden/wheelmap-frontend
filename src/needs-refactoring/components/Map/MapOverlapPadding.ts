...
function subtractRect(a: Rect, b: Rect) {
  ...
  const oneArea =
    one.bottom > one.top && one.right > one.left
      ? (one.bottom - one.top) * (one.right - one.left)
      : 0;
  const twoArea =
    two.bottom > two.top && two.right > two.left
      ? (two.bottom - two.top) * (two.right - two.left)
      : 0;
  const threeArea =
    three.bottom > three.top && three.right > three.left
      ? (three.bottom - three.top) * (three.right - three.left)
      : 0;
  const fourArea =
    four.bottom > four.top && four.right > four.left
      ? (four.bottom - four.top) * (four.right - four.left)
      : 0;
...
export class MapOverlaps {
  ...
  readonly collectPaddings = (map: MapRef, additionalPadding: number) => {
    ...
    for (const element of this.overlapRegions.values()) {
      ...
      if (!remainingRect) {
        console.warn("Degenerate case, ignoring padding calculation", {
          mapRect,
          overlapRegions: this.overlapRegions,
        });
        return additionalPadding;
      }
    ...
    if (remainingRect) {
      ...
      padding.bottom =
        mapRect.bottom - remainingRect.bottom + additionalPadding;
...
export function calculateDefaultPadding() {
  return Math.min(
    Math.max(Math.max(window.innerWidth / 10, window.innerHeight / 10), 20),
    80,
  );
}
