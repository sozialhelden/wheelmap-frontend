...
import type { databaseTableNames } from "~/needs-refactoring/components/Map/filterLayers";
import type { Brand } from "./brand";

export type OSMFeatureNumericId = Brand<
  number,
  "@brand_osm_feature_numeric_id"
>;

export type AnyOSMId =
  | OSMTypedId
...
export type OSMTypedId = Brand<
  `${OSMElementType}${OsmSeperator}${OSMNumericValue}`,
  "@brand_osm_element_value_id"
>;
// eslint-disable-next-line @typescript-eslint/naming-convention
export type OSMTypedId_Deprecated = Brand<
  `${OSMElementType}${OsmSeperator}${OSMNumericValue}`,
  "@brand_osm_element_value_legacy_id"
>;

export type OSMInhouseDatabaseTableName = (typeof databaseTableNames)[number];
export type OSMIdWithTypeAndTableName = Brand<
  `${OSMInhouseDatabaseTableName}${OsmSeperator}${OSMElementType}${OsmSeperator}${OSMNumericValue}`,
  "@brand_osm_key_element_value_id"
>;
...
export type OSMIdWithTableAndContextNames = Brand<
  `${OSMRDFContextName}:${OSMInhouseDatabaseTableName}/${OSMElementType}/${OSMNumericValue}`,
  "@brand_osm_rdf_key_element_value_id"
>;
...
export namespace LegacyOsmId {
  export type NegativeOsmValue = Brand<
    `-${number}`,
    "@brand_deprecated_osm_value"
  >;
  type PositiveOsmValue = `${number}`;

  export type MongoDbOsmValue = Brand<
    `nodes/${string}`,
    "@brand_deprected_mongo_db_osm_value"
  >;
   ...
   * ```
   */
  export type WronglyTypedLegacyOsmId = Brand<
    `nodes/${NegativeOsmValue}` | `nodes/${PositiveOsmValue}`,
    "@brand_deprecated_osm_node_value"
  >;

  export type AnyLegacyOsmId =
    | NegativeOsmValue
    | MongoDbOsmValue
    | WronglyTypedLegacyOsmId;
}
