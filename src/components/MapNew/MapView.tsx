...
import { useMap, useMapState } from "./useMap";
...
export default function MapView(props: IProps) {
  ...
  const {
    ...
    setUnlockedControlMode,
  } = useMapState({
    width,
    height,
  });

  const mapQueryUpdater = useMap();
  ...
  const handleMapClick = useCallback<(event: MapLayerMouseEvent) => void>(
    ...
    [viewState.zoom, mapQueryUpdater]
  );
    ...
    <>
      ...
      <MapProvider>
        <Map
          ...
          onMoveEnd={updateViewportQuery}
          onClick={handleMapClick}
          onMouseDown={setExclusiveControlMode}
          onTouchStart={setExclusiveControlMode}
