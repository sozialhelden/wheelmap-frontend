...
const captions = {
  ...
  infoPageUrl: sourceNameString => t`Öffnen auf ${sourceNameString}`,
  // translator: Button caption in the place toolbar. Navigates to a place's details on an external page.
  editPageUrl: sourceNameString => t`Auf ${sourceNameString} verbessern`
};
...
export default function ExternalInfoAndEditPageLinks(props: Props) {
  ...
  const links = ['infoPageUrl', 'editPageUrl'].map(propertyName => {
    return <SourceLink key={propertyName} properties={properties} knownSourceNameCaption={captions[propertyName]} propertyName={propertyName} />;
  });
