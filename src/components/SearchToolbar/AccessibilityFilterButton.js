...
export default function AccessibilityFilterButton(props: Props) {
  ...
  return <StyledNavLink to={href} className={`${props.className} ${showCloseButton ? 'is-horizontal' : ''} ${props.isActive ? 'is-active' : ''}`} onFocus={props.onFocus} onBlur={props.onBlur} onKeyDown={props.onKeyDown} tabIndex={0} role="button" aria-label={showCloseButton ? t`${caption}-Filter entfernen` : caption}>
