...
export const makeDisplayDataFromPhotonResult = (photonResult: PhotonResultFeature): EnrichedSearchResultDisplayData => {
...
export function mapResultToUrlObject(result: EnrichedSearchResult) {
  ...
  if (result.placeInfo) {
    if (result.placeInfo.properties.sameAs) {
      const osmUrl = result.placeInfo.properties.sameAs.find((url) => url.startsWith('https://www.openstreetmap.org/'))
      if (osmUrl) {
        return {
