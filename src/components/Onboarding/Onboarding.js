...
function Onboarding(props: Props) {
  ...
  const claim = t`Finde und markiere rollstuhlgerechte Orte – weltweit und kostenlos. Mit dem einfachen Wheelmap-Ampelsystem:`;
  // translator: Shown on the onboarding screen. To visit it, open Wheelmap in an incognito window.
  const unknownAccessibilityIncentiveText = t`Hilf mit und markiere Orte!`;
  // translator: Button caption shown on the onboarding screen. To visit it, open Wheelmap in an incognito window.
  const startButtonCaption = t`Okay, los geht’s!`;
  ...
  return <ModalDialog className={props.className} isVisible={props.isVisible} onClose={props.onClose} ariaDescribedBy="wheelmap-claim-onboarding wheelmap-icon-descriptions" ariaLabel={t`Wheelmap-Startbildschirm`}>
    <header>
