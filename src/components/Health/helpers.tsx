...
export const useHealthAPIURL = (options: any) => {
  const {
    ...
    wheelchair,
    healthcare,
    ["healthcare:speciality"]: healthcareSpeciality,
  } = options;
...
export const getFilterOptions = (options: any) => {
  ...
  if (bbox || wheelchair || tags) {
    ...
    const editedWheelchair = `wheelchair=${wheelchair}`;
    return (
      `http://localhost:4000/api/v1/amenities.json?${editedBbox}&${editedWheelchair}&${editedLimit}` +
      `&${editedTags}&geometry=centroid&mode=aggregate&aggregate=count`
    );
  }
  return [];
};
