...
export function OSMTagTableRow(
  ...
  const detailElementsContained = hasDetails && <div style={{ maxWidth: '30rem', padding: '.5rem' }}>
    {detailElements.length === 1 && <StyledMarkdown>{detailElements[0]}</StyledMarkdown>}
    {detailElements.length > 1 && <ul>
  ...
  const displayedValueContent = <>
    {valueIsString
      ? <StyledMarkdown inline={true} style={{ textDecoration: hasDetails ? 'underline dotted' : 'none' }}>{valueElement}</StyledMarkdown>
      : valueElement}
  ...
  const displayedValue = (
    <ValueElement
      ...
      large={true}
    >
      {hasDetails ? <Tooltip content={detailElementsContained} lazy={true} compact={true}>
        {displayedValueContent}
      </Tooltip> : displayedValueContent}
    </ValueElement>
  );
