...
export default class PhotoUploadCaptchaToolbar extends React.Component<Props, State> {
  ...
  render() {
    ...
    const mailHref = `mailto:photos@wheelmap.org?subject=${t`Wheelmap-Fotoupload`}&body=${t`Liebes Wheelmap-Team, ich habe von diesem Ort ein Foto gemacht:`}`;
    ...
    const captions = {
      ...
      help: t`Bitte gib diese Buchstaben/Zahlen ein.`,
      ...
      explanation: t`Dann wissen wir, dass du ein Mensch bist.`,
      // translator: Shown below the captcha input field before uploading an image. We try to avoid the word 'captcha' because normal users might not know what it means.
      emailHint: t`Wenn Du das hier nicht lesen oder eingeben kannst, sende uns Deine Fotos per E-Mail!`,
      ...
      startingUpload: t`Danke! Starte Upload…`,
      ...
      unreachable: t`Server nicht erreichbar. Probier es später nochmal oder schreib uns, wenn der Fehler weiterhin vorkommt!`,
      ...
      invalid: t`Sorry, das hat nicht geklappt! Bitte überprüfe alle Zeichen/Buchstaben und probier's dann nochmal.`
    };
