...
const surfaceLayerIds = new Set(["osm-pedestrian-outline", "osm-surface-fill"]);

const isOsmLayer = (layer: mapboxgl.Layer) => layer.id.startsWith("osm-");
const isPublicTransportLayer = (layer: mapboxgl.Layer) =>
  publicTransportLayerIds.has(layer.id);
const isBuildingLayer = (layer: mapboxgl.Layer) =>
  buildingLayerIds.has(layer.id);
...
export function filterLayers({
  ...
  for (let i = 0; i < layers.length; i += 1) {
    ...
    const accessibilityCloudLayer = {
      ...enhancedLayer,
      id: `${enhancedLayer.id}-ac`,
      source: "ac:PlaceInfo",
