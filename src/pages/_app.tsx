...
import composeContexts, {
  type ContextAndValue,
} from "../lib/context/composeContexts";
...
import {
  addToEnvironment,
  getEnvironment,
} from "../lib/util/globalEnvironment";
...
import ExpertModeContext, {
  ExpertModeContextProvider,
} from "../components/App/MainMenu/useExpertMode";
...
async function retrieveAppByHostname(
...
function determinePreferredLanguageTags(
  req: IncomingMessage,
  res: ServerResponse,
): ILanguageSubtag[] {
  ...
  const languageTagStrings = req
    ? (acceptLanguageHeader &&
        parseAcceptLanguageString(acceptLanguageHeader)) || ["en"]
...
const getInitialProps: typeof NextApp.getInitialProps = async (appContext) => {
  ...
  const languageTags =
    req && res
      ? determinePreferredLanguageTags(req, res)
      : window.navigator.languages.map(parseLanguageTag);
  ...
  const hostnameAndPort =
    query.appId || (req ? req.headers.host : window.location.hostname);
  if (typeof hostnameAndPort !== "string") {
    throw new Error("Please supply only one appId query parameter.");
  }
  const environment = getEnvironment();
  const app = await retrieveAppByHostname(
    Object.keys(environment).length > 0 ? environment : environmentVariables,
    hostnameAndPort,
    query,
  );
